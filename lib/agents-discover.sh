...
# Skips _*, README.md, and uppercase doc files (REFERENCES, HANDOFF-TEMPLATES).
# Product agents are lowercase kebab-case. Prints one name per line, sorted.
_gh_list_agent_names() {
  ...
  for f in "${root}/agents"/*.md; do
    ...
    # Doc indexes in agents/ are TITLE-CASE; Grok agent types are lowercase.
    [[ "${name}" == *[A-Z]* ]] && continue
    names+=("${name}")
  done
  if [[ ${#names[@]} -eq 0 ]]; then
