...
post_complete_actions() {
  ...
  # Detect whether the user has a full overlay tree (agents/skills/bin)
  local has_full_tree=0
  local root=""
  for d in "${SCRIPT_DIR:-}" "${GROKHUNTER_HOME:-}" "${HOME}/GrokHunter"; do
    if [[ -n "${d}" && -d "${d}/bin" && -d "${d}/skills" && -d "${d}/agents" ]]; then
      root="${d}"
      has_full_tree=1
      break
    fi
  ...
  if [[ ${has_full_tree} -eq 0 ]]; then
    msg -a "  ${Y}────────────────────────────────────────────────────────────${S}"
    msg -a "  ${Y}Full agents / skills / menu not installed yet${S}"
    msg -a "  ${Y}(one-liner only installed the core modules)${S}"
    echo
    msg -a "  To unlock Coding Team, personas, Kali menu & shortcuts:"
    msg -a "    git clone https://github.com/FineComputer14451/GrokHunter.git ~/GrokHunter"
    msg -a "    cd ~/GrokHunter"
    msg -a "    grokhunter skills install          # or: bash install.sh --overlay-only --with-completions"
    msg -a "    grokhunter doctor"
    msg -a "  ${Y}────────────────────────────────────────────────────────────${S}"
    echo
  else
    msg -a "  ${T}Full overlay detected:${S} ${root}"
    msg -a "  Agents / skills / menu already available."
    echo
  fi
