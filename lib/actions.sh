...
post_complete_actions() {
  ...
  msg -a "    sudo apt update && sudo apt install -y build-essential git python3"
  echo
  if command -v grok &>/dev/null; then
    msg -a "  ${T}Grok Build:${S}"
    msg -a "    grok               ${S}# Interactive pair session${S}"
    msg -a "    grok -p \"task\"     ${S}# Headless mode${S}"
  fi
  if command -v aider-grok &>/dev/null || [[ -x ${HOME}/venv-aider/bin/aider ]]; then
    msg -a "  ${T}Aider:${S}"
    msg -a "    aider-grok         ${S}# Git-native pair-programmer (xAI)${S}"
  fi
  echo
  msg -a "${P}Happy coding — your AI-powered rootless lab is ready!${S}"
  msg -a "${P}════════════════════════════════════════════════════════════${S}"
}
