#!/usr/bin/env bash
# Cloudflare quick tunnel in front of Grok Build Connect MCP.
set -euo pipefail
...
elif [[ -x "$HOME/.grokhunter-grok-build-connect/bin/cloudflared" ]]; then
  CF="$HOME/.grokhunter-grok-build-connect/bin/cloudflared"
...
if ! curl -fsS "http://127.0.0.1:${PORT}/healthz" >/dev/null 2>&1; then
  echo "MCP not healthy on :${PORT} — start it first (tmux attach -t gbc)" >&2
  exit 1
fi

echo "Starting quick tunnel → http://127.0.0.1:${PORT}"
echo "Copy the https://….trycloudflare.com URL, append /mcp"
echo "Bearer token file: ~/.grokhunter-grok-build-connect/token (do not paste into chat)"
