...
PY
  then
    die "python strip failed for $CFG"
  fi
}
...
_append_v9_surface() {
  ...
  {
    cat "$CFG" || die "cannot read $CFG for merge"
    echo ""
    ...
    printf '%s\n' "$blocks"
  } >"$TMP_OUT" || die "failed writing temp config $TMP_OUT"

  grep -q "$MARKER_BEGIN" "$TMP_OUT" || die "temp config missing marker"
  grep -q '^\[model\.' "$TMP_OUT" || die "temp config missing [model.*]"

  mv -f "$TMP_OUT" "$CFG" || die "failed to install new config at $CFG"
  TMP_OUT=""
