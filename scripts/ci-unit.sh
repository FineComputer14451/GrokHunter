...
bash bin/grokhunter help | grep -q credits || die "help missing credits"
bash bin/grokhunter menu help | grep -q install || die "menu help missing install"
bash bin/grokhunter credits | grep -qi jorexdeveloper || die "credits missing jorexdeveloper"
bash bin/grokhunter credits | grep -qi Termux || die "credits missing Termux"
bash bin/grokhunter credits | grep -qiE 'Kali|Offensive' || die "credits missing Kali/OffSec"
bash bin/grokhunter credits | grep -qi xAI || die "credits missing xAI"
