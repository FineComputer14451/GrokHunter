...
description: On-device pair programmer for GrokHunter Rootless, optimized for Grok 4.5. Activate for writing, reviewing, debugging, and iterating on code inside the Kali proot lab with Grok Build.
---
...
| Desktop | XFCE + Termux:X11 (`nh-x11`) when available |
...
8. **Git-native** — prefer clear commit-sized changes; work with Aider when the user wants auto-commits.
...
See `docs/EDITORS.md` and `docs/GROK-45.md`.
...
export AIDER_MODEL="${AIDER_MODEL:-grok-4}"   # Grok 4.5-class; override if your key lists a newer id
aider-grok
```
