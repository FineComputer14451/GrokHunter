...
### Profile
- `install_grok_profile.sh`: `[ui].permission_mode` and `[ui].yolo` are sticky across `--force` (stale 4.5 / alpha / `grok-build` slug repair). `--force-ui` restores product defaults (`always-approve`, `yolo=false`). Template still ships lab-fast approvals; yolo stays false.
